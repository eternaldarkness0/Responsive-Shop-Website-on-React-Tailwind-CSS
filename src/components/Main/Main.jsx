import React from 'react'
import Image1 from '../../assets/sale.png'
import Image2 from '../../assets/shopping.png'
import Image3 from '../../assets/woman1.png'
import Slider from 'react-slick'


const ImageList = [
    {
        id: 1,
        img: Image1,
        title: "70% off on all Products Sale",
        description: 'Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.',
    },

    {
        id: 2,
        img: Image2,
        title: "30% off on all Women's Wear",
        description: 'Lorem, ipsum dolor sit amet consectetur adipisicing elit. Impedit obcaecati harum inventore sed.',
    },

    {
        id: 3,
        img: Image3,
        title: "Up to 50% off on all Men's Wear",
        description: 'Lorem, ipsum dolor sit amet consectetur adipisicing elit. Impedit obcaecati harum inventore sed.',
    },
];

export const Main = ({handleOrderBtn}) => {

let settings = {
    dots: false,
    arrows: false,
    infinite: true,
    speed: 800,
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 4000,
    cssEase: 'ease-in-out',
    pauseOnhover: false,
    pauseOnFocus: true,
};

return (
    <main id='home' className='relative overflow-hidden min-h-[550px]
    sm:min-h-[650px] bg-gray-100 flex justify-center items-center dark:bg-gray-950 dark:text-white duration-200'>

        {/* background  */}
        <div className='h-[700px] w-[700px] bg-primary/40 absolute -top-1/2 right-0 rounded-3xl rotate-45'>
        </div>

        <div className='container pb-8 sm:pb-0'>

            <Slider {...settings}>

                {/* slider text */}
                {ImageList.map((data) => (
                <div key={data.id}>
                    <div className='grid grid-cols-1 sm:grid-cols-2 px-2'>
                        <div
                        className='flex flex-col justify-center gap-4 pt-12 sm:pt-0 text-center sm:text-left order-2 sm:order-1 relative'>
                            <h1
                            data-aos='fade-up'
                            data-aos-once='true'
                            data-aos-delay='100'
                            className='text-5xl sm:text-6xl lg:text-7xl font-bold'
                            >{data.title}</h1>
                            <p
                            data-aos='fade-up'
                            data-aos-once='true'
                            data-aos-delay='200'
                            className='text-sm'
                            >{data.description}</p>
                            <div
                            data-aos='fade-up'
                            data-aos-once='true'
                            data-aos-delay='300'
                            >
                                <button
                                className='bg-gradient-to-r from-primary to-secondary hover:scale-105 duration-200 text-white py-2 px-4 rounded-full'
                                onClick={handleOrderBtn}
                                >
                                Order Now
                                </button>
                            </div>
                        </div>

                        {/* slider img */}
                        <div className='order-1 sm:order-2'>
                            <div
                            data-aos='zoom-in'
                            data-aos-once='true'
                            className='relative z-10'>
                                <img
                                className='w-[300px] h-[300px] sm:h-[450px] sm:w-[450px] sm:scale-105 lg:scale-120 object-contain mx-auto'
                                src={data.img} alt="img" />
                            </div>
                        </div>
                    </div>
                </div>
                ))}

            </Slider>

        </div>
    </main>
)
}
